...
export const loadArticleSiteMap = async () => {
  let finalSlugsArray
  database
    .ref()
    .child('site/articles')
    ...
    .then(async snapshot => {
      if (snapshot.exists()) {
        const tempArticlesSlugs = snapshot.val()
        if (tempArticlesSlugs.params) {
          finalSlugsArray = tempArticlesSlugs.map(slug => slug.params)
        }
      }
    })
  return finalSlugsArray
}
